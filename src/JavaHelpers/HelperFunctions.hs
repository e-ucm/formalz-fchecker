...
lookupType decls _ (Name (Ident s@('$':_) : idents))
  -- Names starting with a '$' symbol are generated and represent the return variable of a function
  = getFieldType decls (getReturnVarType decls s) (Name idents)
lookupType _ _ (Name (Ident ('#':_) : _))
  -- Names starting with a '#' symbol are generated and represent a variable introduced by handling operators
  = PrimType undefined
lookupType decls env (Name idents)
  = case lookup (Name [head idents]) env of
      Just t  -> getFieldType decls t (Name (tail idents))
      Nothing -> PrimType IntT
        -- used for lambda arguments
        -- non-int types have to be declared in the method's formal parameters
        {- error ("can't find type of " ++ prettyPrint (Name idents) ++ " in " ++ prettyprintTypeEnv env) -}

-- | Gets the type of a field of an object of given type
getFieldType :: [TypeDecl] -> Type -> Name -> Type
getFieldType _ t (Name [])
  = t
getFieldType _ _ (Name [Ident "length"])
  = PrimType IntT
getFieldType decls (RefType (ClassRefType t)) (Name (f:fs))
  = getFieldType decls (getFieldTypeFromClassDecl (getDecl t decls) f) (Name fs)
    where
        getFieldTypeFromClassDecl :: ClassDecl -> Ident -> Type
        getFieldTypeFromClassDecl (ClassDecl _ _ _ _ _ (ClassBody decls))
          = getFieldTypeFromMemberDecls decls

        getFieldTypeFromMemberDecls :: [Decl] -> Ident -> Type
        getFieldTypeFromMemberDecls [] _
          = error "getFieldTypeFromMemberDecls"
        getFieldTypeFromMemberDecls (MemberDecl (FieldDecl mods t (VarDecl varId _ : vars)) : decls) id
          = if getId varId == id then t else getFieldTypeFromMemberDecls (MemberDecl (FieldDecl mods t vars) : decls) id
        getFieldTypeFromMemberDecls (_ : decls) id
          = getFieldTypeFromMemberDecls decls id

         -- Gets the class declaration that matches a given type
        getDecl :: ClassType -> [TypeDecl] -> ClassDecl
        getDecl t@(ClassType [(ident, typeArgs)]) (x:xs) =
          case x of
            ClassTypeDecl decl@(ClassDecl _ ident' _ _ _ _)
              -> if ident == ident' then decl else getDecl t xs
            _ -> getDecl t xs
...
getMethodClassType decls id = head $ concatMap (`getMethodTypeFromClassDecl` id) decls
    where
        ...
        getMethodTypeFromClassDecl (ClassTypeDecl (ClassDecl _ className _ _ _ (ClassBody decls))) id
          = getMethodTypeFromMemberDecls (RefType (ClassRefType (ClassType [(className , [])]))) decls id
        ...
        getMethodTypeFromMemberDecls t [] _
          = []
        getMethodTypeFromMemberDecls t (MemberDecl (MethodDecl _ _ _ id' _ _ _ _) : decls) id
          = if id' == id then [t] else getMethodTypeFromMemberDecls t decls id
        getMethodTypeFromMemberDecls t (_ : decls) id
          = getMethodTypeFromMemberDecls t decls id
...
extendEnv env decls methodId =
  case getMethodType decls methodId of
    Nothing ->
      (Name [Ident "*obj"], getMethodClassType decls methodId) : env
    Just (RefType _)  ->
      (Name [Ident "returnValue"], returnValueType) :
        (Name [Ident "returnValueVar"], returnValueType) :
          (Name [Ident "*obj"], getMethodClassType decls methodId) : env
    Just t  ->
      (Name [Ident "returnValue"], t) :
        (Name [Ident "returnValueVar"], t) :
          (Name [Ident "*obj"], getMethodClassType decls methodId) : env
...
returnValueType =
  RefType (ClassRefType (ClassType [(Ident "ReturnValueType", [])]))
...
getReturnVarType decls s =
  ...
  fromMaybe (PrimType undefined) $
    getMethodType decls (Ident (takeWhile (/= '$') (tail s)))

-- Increments the call count for a given method
incrCallCount :: CallCount -> Ident -> CallCount
incrCallCount [] id
  = [(id, 1)]
incrCallCount ((id', c):xs) id
  = if id == id' then (id', c + 1) : xs else (id', c) : incrCallCount xs id
...
getMethod classTypeDecls methodId =
  (\(b, _, _) -> b) <$> getMethod' classTypeDecls methodId

-- Gets the return type of a method
getMethodType :: [TypeDecl] -> Ident -> Maybe Type
getMethodType classTypeDecls methodId =
  getMethod' classTypeDecls methodId >>= (\(_, t, _) -> t)

-- Gets the parameter declarations of a method
getMethodParams :: [TypeDecl] -> Ident -> Maybe [FormalParam]
getMethodParams classTypeDecls methodId =
  (\(_, _, params) -> params) <$> getMethod' classTypeDecls methodId
...
getMethod' classTypeDecls methodId =
    case concatMap searchClass classTypeDecls of
      r:_ -> Just r
      []  -> Nothing -- Library function call
  where
    searchClass (ClassTypeDecl (ClassDecl _ _ _ _ _ (ClassBody decls)))
      = searchDecls decls
    searchClass _
      = []

    searchDecls (MemberDecl (MethodDecl _ _ t id params _ _ (MethodBody (Just b))):_)
      | methodId == id
      = [(StmtBlock b, t, params)]
    searchDecls (MemberDecl (ConstructorDecl _ _ id params _ (ConstructorBody _ b)):_)
      | methodId == toConstrId id
      = [(StmtBlock (Block b), Just (RefType (ClassRefType (ClassType [(id, [])]))), params)]
    searchDecls (_:decls)
      = searchDecls decls
    searchDecls []
      = []
...
getMainMethod classTypeDecls =
  fromJust' "getMainMethod" $ getMethod classTypeDecls (Ident "main")
...
-- Checks if the var is introduced.
-- Introduced variable names start with '$' voor return variables of methods and '#' for other variables
...
resetVarMethodInvokesCount = do
  writeIORef varMethodInvokesCount []
  readIORef varMethodInvokesCount
...
fieldAccess e (Name [id])
  = PrimaryFieldAccess e id
fieldAccess e (Name (id:ids))
  = fieldAccess (FieldAccess (PrimaryFieldAccess e id)) (Name ids)
fieldAccess _ _
  = error "FieldAccess without field name"
...
adjustCallCount__ (ExpName (Name [Ident s])) newCount =
  ExpName (Name [Ident (s0 ++ key ++ show newCount)])
    where
      key = "___retval"
      k = fromJust' "adjustCallCount__" $ findInfix key s
      s0 = take k s

getMethodId__ (ExpName (Name [Ident s])) = Ident (take k s)
    where
      key = "___retval"
      k = fromJust' "getMethodId__" $ findInfix key s
...
normalizeInvocationNumbers e = normalize e
  where
    ...
    normalizeName e@(ExpName (Name [ident])) =
      if k>=0 then adjustCallCount__ e (k `mod` m) else e
        where
          k = getCallCount__ e
    ...
    countCallVars calls e = case e of
      -- base case
      ename@(ExpName n) ->
        let k = getCallCount__ ename
            methodid = getMethodId__ ename
        in if k >= 0 then registerInvocation (methodid,k) calls
                     else calls
