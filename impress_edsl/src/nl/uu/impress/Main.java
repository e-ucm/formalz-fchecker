...
public class Main {
    ...
    public static void swap2d(int[][] a, int i, int j) {
      pre(i >= 0 && j >= 0);
      swap(a, i, j);
      post(a[j][i] == oldai && a[i][j] == oldaj);
    }
    public static void sort2d(int[][] a, int[][] sa) {
      pre(a != null);
    ...
    public static void max2d_1(int[][] a) {
      post(
        forall(a, i ->
          forall(a[i], j ->
            a[i][j] <= retval)));
    }
    public static void max2d_2a(int[][] a) {
      post(
        forall(a, i ->
          forall(a[i], j ->
            !(a[i][j] >= retval) || !(a[i][j] != retval))));
    }
    public static void max2d_2b(int[][] a) {
      post(
        forall(a, i ->
          !exists(a[i], j ->
            a[i][j] > retval)));
    ...
    public static void sortbylength_1(int[][] a, int[][] sa) {
        pre(a != null);
    ...
    public static void sortbylength_2a(int[][] a, int[][] sa) {
        pre(a != null);
        pre(forall(a, i -> a[i] != null));
        int[][] sa = sort_by_length(a);
        post(
          forall(sa, i ->
            forallr(sa, (2*i) - i, (2*sa.length) - sa.length, j ->
              !(sa[i].length > sa[j].length)
        )));
    }
    // Solvable due to heuristics in `Normalizer.hs`
    public static void sortbylength_2b(int[][] a, int[][] sa) {
        pre(a != null);
        pre(forall(a, i -> a[i] != null));
        int[][] sa = sort_by_length(a);
        post(
          forall(sa, i ->
            !existsr(sa, (2*i) - i, (2*sa.length) - sa.length, j ->
              sa[i].length > sa[j].length
        )));
    }
}
